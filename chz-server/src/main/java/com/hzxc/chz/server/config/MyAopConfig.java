...
public class MyAopConfig {
    ...
    public void FindRequestMapping() {
        try {
            // 包下面的类
            Set<Class<?>> clazzs = getClasses("com.hzxc.chz.server.web");
            if (clazzs == null) {
                return;
            }
                ...
                for (Class<?> cls : clazzs) {
                    ...
                    for (Method m : ms) {
                        ...
                        if (m.getAnnotation(AopTest.class) != null
                            ...
                            MyAopInfo info = new MyAopInfo();
                            info.setClassInstance(codeInstance);
                            info.setCodeMethod(m);
                            info.setMethodParameters(needparameters);

                            if (m.getAnnotation(AopTest.class) != null) {
                                aopInfoMap.put(m.getName(), info);
                            }
