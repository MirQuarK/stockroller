...
public interface GxqCommissionRepository extends JpaRepository<GxqCommission, Long> {
    @Query(value = "select * from gxq_commission where user_id = ?1 and create_time BETWEEN ?2 and ?3 limit ?4,?5"
            ,nativeQuery = true)
    List<GxqCommission> getByTimePage(int userId, String start, String end, int sindex, int index);

    @Query(value = "select count(*) from gxq_commission where user_id = ?1 and create_time BETWEEN ?2 and ?3"
            ,nativeQuery = true)
